...
/** Returns the channel keys for a prospect's messages (new format only) */
function getChannelKeys(prospect: ProspectRecord): string[] {
  if (!prospect.messages?.messages) return []
  return prospect.messages.messages.map((m) => m.templateId)
}

/** Returns display label for a channel key */
function channelDisplay(key: string, prospect: ProspectRecord): string {
  if (!prospect.messages?.messages) return key.slice(0, 2).toUpperCase()
  const msg = prospect.messages.messages.find((m) => m.templateId === key)
  if (!msg) return key.slice(0, 2).toUpperCase()
  // Abbreviate: first 2 chars of template name
  return msg.templateName.slice(0, 2).toUpperCase()
}

/** Check if messages are in the new format (have .messages array) */
function hasNewFormat(messages: GeneratedMessages | null): boolean {
...
export default function ProspectsPage() {
  ...
  function handleFeedback(
    ...
    const next: MessageFeedbackMap = {
      ...p.feedback,
      [templateId]: p.feedback[templateId] === value ? null : value,
    }
    mutate(prospectId, { feedback: next })
  }
          ...
          <div className="w-72 flex-shrink-0 overflow-y-auto space-y-2">
            {filtered.length === 0 ? (
              ...
              filtered.map((p) => {
                ...
                const channelKeys = getChannelKeys(p)
                return (
                  <button
                    key={p.id}
            ...
            <div className="flex-1 min-w-0 rounded-xl border border-border bg-surface shadow-soft overflow-hidden flex flex-col">
              ...
              <div className="flex-1 min-h-0 overflow-y-auto">
                <div className="px-6 py-4 space-y-5">
                  {/* Channel tracker — only show for new format */}
                  {hasNewFormat(selectedProspect.messages) && getChannelKeys(selectedProspect).length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Channels sent</p>
                      <div className="flex gap-2 flex-wrap">
                        {getChannelKeys(selectedProspect).map((ch) => (
                          <button
                            key={ch}
                            onClick={() =>
                              mutate(selectedProspect.id, {
                                channels: {
                                  ...
                                  [ch]: !selectedProspect.channels?.[ch],
                                },
                          ...
                          >
                            ...
                            {channelDisplay(ch, selectedProspect)}
                          </button>
                        ))}
                      </div>
                  ...
                  {selectedProspect.messages ? (
                    <div>
                      ...
                      {hasNewFormat(selectedProspect.messages) ? (
                        <MessageTabs
                          messages={selectedProspect.messages}
                          onRegenerate={() => handleRegenerate(selectedProspect)}
                          isGenerating={isRegenerating}
                          feedback={selectedProspect.feedback}
                          onFeedback={(templateId, value) => handleFeedback(selectedProspect.id, templateId, value)}
                        />
                      ) : (
                        <div className="rounded-lg border border-dashed border-border p-6 text-center">
                          <p className="text-sm text-muted mb-2">Messages are in an older format.</p>
                          <p className="text-xs text-muted">Regenerate to update to the new template format.</p>
                          <button
                            onClick={() => handleRegenerate(selectedProspect)}
                            disabled={isRegenerating || !selectedProspect.lastPayload}
                            className="mt-3 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary-hover transition-colors disabled:opacity-50"
                          >
